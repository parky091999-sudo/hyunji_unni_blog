...
def _struct_etf_individual(cfg: dict, market_label: str, has_dividend: bool = False) -> str:
    ...
    dividend_block = (
        ...
        # ★[사진3] 바로 앞 줄은 반드시 아래 '독립 문장' 하나여야 한다(불릿·표 금지). 이미지 앵커로
        #  쓰이므로 불릿 문장 중간에 차트가 끼어드는 사고(DIVO 실발행)를 막는다. 문구 고정.
        ...
        # ★[사진4] 바로 앞 줄도 아래 독립 문장 고정(불릿·표 금지) — 앵커 안정화.
    ...
    return (
        ...
        "레버리지·인버스면 단기 접근 원칙. 팩트 데이터에 근거해서 작성. "
        "팩트 데이터에 '최근뉴스'가 있으면 이 ETF·기초지수 관련 최신 이슈(금리·배당 발표·지수 이벤트 등)를 "
        "체크포인트에 반영하라. 불릿 3~4줄.)\n"
        "\n[소제목] 자주 묻는 질문\n"
        "(★FAQ 질문 3개는 이 ETF만의 특징을 반영해 매번 다르게 만들어라 — 아래 괄호는 방향 예시일 뿐 "
        "그대로 베끼지 마라. 이 상품 고유의 성격(커버드콜/레버리지/배당성장/월배당/기초지수/섹터 쏠림 등)과 "
        "오늘 팩트(등락·배당수익률·백테스트)를 반영한 구체적 질문을 써라. 매 글 똑같은 '초보자도 되나요/"
        "보수는 왜 보나요/지금 사도 될까요' 3종 세트 반복 금지.)\n"
        "[FAQ시작]\n"
        "Q: (이 ETF 고유 특징 관련 — 예: 커버드콜이면 상승장에서 손해인지, 레버리지면 장기보유 왜 위험한지 등)\n"
        "A: (성격·구조 기반 구체 답변)\n"
        "Q: (이 상품의 배당·비용·세금 등 실전 궁금증 — 오늘 수치 반영)\n"
        "A: (데이터 기반 쉬운 설명)\n"
