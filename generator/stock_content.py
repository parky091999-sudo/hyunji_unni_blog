...
_COMMON_RULES = (
    ...
    "재해석해 단정하지 말고 '이런 보도가 있었다' 수준으로 중립 인용. "
    "★본문에 뉴스 URL·외부 링크를 절대 넣지 마라('http', '[가운데] URL' 금지) — 출처는 '연합뉴스 보도'처럼 "
    "매체명 텍스트로만 인용한다. 클릭되는 외부 링크카드는 독자를 블로그 밖으로 내보내 체류시간·유입 손실을 "
    "유발하므로 금지. 뉴스 없으면 이 항목 무시.\n"
...
def _struct_single_stock(cfg: dict, has_financials: bool = False, has_naver_chart: bool = False) -> str:
    return (
        ...
        "'최근뉴스'가 있으면 매체명을 '연합뉴스 보도에 따르면'처럼 텍스트로만 인용하라 — URL·링크카드 금지(외부 이탈 방지). 불릿 3~4줄.)\n"
...
def _research_material_brief(stock_name: str, api_key: str, reason: str = "",
    ...
    try:
        ...
        prompt = (
            ...
            "5) 출처: 근거 기사 제목과 매체명 2~3개 (URL 말고 매체명만 — 본문에 외부 링크를 넣지 않음)\n"
...
def _research_market_brief(event_desc: str, api_key: str) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            "6) 출처: 근거 기사 제목과 매체명 2~3개 (URL 말고 매체명만 — 본문에 외부 링크를 넣지 않음)\n"
