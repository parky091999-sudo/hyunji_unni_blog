...
CATEGORIES: dict[str, dict] = {
    ...
    # ── 보험 (고CPC, 건강↔돈 브릿지) ──────────────────────────────
    # 2026-07-06 재편: 국민연금(공적연금, 주제표류) 제거 + 20→42개 확장
    # (시즌형은 DataLab 트렌딩이 검색량으로 제철에 알아서 우선 선택)
    "보험": {
        "name": "보험",
        "keywords": [
            # 실손·건강
...
def pick_keyword_for_blog_category(blog_category: str, exclude: set | None = None) -> dict:
    """
    ...
    month = datetime.now(KST).month
    recent = _get_recent_keywords(days=30)
    # ★호출측(정보성/gov)의 자기 카테고리 이력 — _HISTORY_PATH(살림 post_history.json)엔
    # 정보성 발행이 안 남아 30일 회피가 뚫렸음(보험 '국민연금' 이틀 연속 등 실사고 2026-07-06).
    # 풀 필터 단계에서 제외해야 DataLab 트렌딩이 같은 키워드를 반복 반환하지 못한다.
    if exclude:
        recent = recent | set(exclude)
