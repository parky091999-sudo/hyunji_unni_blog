...
TECH_BODY_MIN = 900  # 속보 브리핑은 본래 짧음 — 하한은 안전망, 목표 길이는 프롬프트로 유도
...
[구조 — 속보·루머형]
[사진1]
(도입 2~3줄. 첫 문장에 핵심 소식 즉시. "무엇이 알려졌다"를 바로.)
...
[소제목] 무슨 소식인가
(유출·발표 내용을 짧은 줄로. 출처 성격 명시: "업계에 따르면", "유출 정보로는")

[소제목] 핵심 스펙·일정 한눈에
(표로 정리. 반드시 3열 이내, 각 셀 12자 이내, 빈칸 금지.)
[표시작]
항목 | 내용
칩셋 | ~
출시 예상 | ~
...
[소제목] 왜 중요한가
(이 소식이 소비자·시장에 주는 의미 2~3줄. 단정 금지.)
...
[구조 — 쉬운 해설형]
[사진1]
(도입 2~3줄. 독자가 궁금해하는 지점을 질문처럼 던지고 쉽게 답하겠다 예고.)
...
[구조 — 꿀템 큐레이터형]
[사진1]
(도입 2~3줄. 어떤 니즈의 제품을 왜 추천하는지. 가격·가성비 후킹.)
...
def generate_tech_post(api_key: str, fmt: str = "breaking", topic: dict | None = None) -> dict | None:
    ...
    user_msg = news_block + (
        ...
        "- ★본문 1,400~2,000자. 각 소제목마다 2~4줄로 충실히 — 배경·맥락·소비자 영향을 담아 밀도를 채워라(빈약한 한 줄 금지).\n"
