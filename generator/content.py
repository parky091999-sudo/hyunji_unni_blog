...
def _parse_response(raw: str) -> dict | None:
    try:
        ...
        if body_start is not None:
            ...
            # 마크다운/기호 제거 (Gemini가 무시하고 생성하는 경우 후처리)
            body = re.sub(r"\*\*(.+?)\*\*", r"\1", body)       # **굵게** → 굵게
            body = re.sub(r"__(.+?)__", r"\1", body)            # __밑줄__ → 밑줄
            body = re.sub(r"^[*\-•]\s+", "", body, flags=re.MULTILINE)  # 줄 앞 * - • 제거
            body = re.sub(r"[✔★○□◆◇▶●►✓➡]", "", body)        # 특수기호 제거
            body = re.sub(r"\n{3,}", "\n\n", body)              # 3줄 이상 빈줄 → 2줄
