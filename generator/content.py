...
5. 이미지: 본문 흐름에 맞게 [사진N] 마커를 유연하게 삽입 (고정 7개 폐지, 글 흐름상 자연스럽게 최소 2개 ~ 최대 4개 권장).
...
- 구조: 실생활 문제상황 공감 도입부 → [사진1] → 원인 분석 및 해결책 제안 (소제목1) → 구체적 시도 과정(2~3단계) 및 [사진2] → 비교/추천표 ([표시작]...[표끝]) → 소감 및 주의사항 → [사진3] → 마무리
- 사진 개수: 3장 내외
- 필수 요소: 비교표

▶ [패턴 B] 요리/레시피 (정보+레시피형)
- 구조: 완성 요리 시각 묘사 도입부 → [사진1] → 준비 재료 및 분량 표 ([표시작]...[표끝]) → 조리 단계 상세 설명(소제목1 & 1. 2. 3. 번호) 및 [사진2] → 조리 시 실패하기 쉬운 포인트 및 해결책 (소제목2) → [사진3] → 보관법 및 요리 후기 마무리 (※ 요리 글에는 FAQ를 넣지 마라)
- 사진 개수: 3~4장
- 필수 요소: 재료 표
...
▶ [패턴 D] 일상/리뷰 (경험 위주형)
- 구조: 스토리 중심 도입부 (장면/에피소드) → [사진1] → 상세 여정 및 감정 묘사 (소제목1) → [사진2] → 직접 경험한 솔직한 평점/단점 인정 (소제목2) → [사진3] → 개인 소감 및 다음 글 예고 마무리 (※ 일상/단순리뷰 글에는 표나 FAQ를 절대 넣지 마라)
- 사진 개수: 3장 내외
...
- ★중요 (AEO 지식스니펫 노출):★ 모든 소제목 바로 아래 첫 단락의 1~2문장은 질문이나 소제목 주제에 대한 결론/정의 형식으로 명확하게 즉시 답변할 것 (서술을 끌거나 미루는 AI 상투어 절대 금지).
...
═══════════════════════════════════════
첫 1~2단락이 다른 글과 똑같은 천편일률 도입부 금지:
- "혹시 ~하고 계신가요? / 아니면 저처럼 ~?"
- "솔직히 저도 처음엔 ~ / 근데요, 몇 번의 시행착오 끝에 ~"
- "오늘은 제가 ~ 알려드릴게요 / 이 글 하나만 잘 읽어두시면 ~"
첫 문장에 '안녕하세요', '오늘은', '혹시 ~신가요'는 절대 금지.

도입부는 아래 스타일 중 하나로(또는 섞어) 자연스럽게 시작:
- 구체적 장면/시간 묘사 — "어제 저녁 8시, 싱크대 앞에서 또 한숨 쉬다가요."
- 의외의 숫자/사실 먼저 — "이거 하나 바꿨더니 한 달 전기세가 1만 2천원 줄었어요."
- 실패담 자백 — "저 이거 1년을 잘못된 방법으로 쓰고 있었더라고요. 진짜 어이없게."
- 결론부터(역두괄식) — "결론부터 말하면, 답은 다이소 2천원짜리였어요."
- 남편/생활 에피소드 — "남편이 '이거 왜 이래?' 묻는 순간 깨달았어요."
