...
[작성 원칙]
══════════════════════════════════════════
1. 정확성 최우선: 모든 금액·조건·기간은 2026년 기준으로 정확하게
2. 수치 필수: 나이·소득·금액·기간 모두 숫자로 명시
3. 표는 정확히 3개: 핵심요약표 / 신청자격표 / 지원금액표
4. 표는 반드시 2열(항목|내용 또는 구분|금액) — 3열 금지 (모바일 깨짐)
...
def generate_gov_post(
    ...
    user_msg = (
        ...
        + "\n- 표 정확히 3개: [표시작]...[표끝] 마커 3쌍 필수."
        + "\n- FAQ 정확히 1개: [FAQ시작]...[FAQ끝] 마커 1쌍 필수."
        + "\n- 표는 반드시 2열(항목|내용 또는 구분|금액) — 3열 금지."
        + "\n- 본문 2,000~2,500자. 단락은 2~3줄 이내."
        + "\n- 제목에 반드시 구체적 금액·수치 포함."
        + "\n- 이미지 3장: [사진1] 맨 위(헤더), [사진2] 자격 섹션 뒤, [사진3] 신청방법 뒤."
    )
