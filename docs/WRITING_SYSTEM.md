...
1. **표 2열화 — SE ONE 열삭제 자동화 실패 (2026-06-30 확인)**
   - 네이버 SE ONE은 표 삽입 시 **크기 그리드 없이 기본 3열 표**를 만든다 → 2열 데이터는 3번째 열이 빈칸으로 남아 모바일에서 가로 깨짐(§5·§6 위반).
   - `poster/naver_blog.py::_delete_table_last_col`이 `bounding_box()` + `page.mouse.click(right)` 우클릭으로 '열 삭제'를 시도하나 **SE ONE 셀 컨텍스트 메뉴가 합성 클릭에 안 떠서 실패**(gov draft run 28410406234에서 3회 모두 "열 삭제 0/1 후 중단"). 이전 2방식(nth click, JS dispatchEvent)도 실패.
   - **다음 조치**: 로컬 headed 세션(시스템 Chrome+쿠키, 한국IP)으로 SE ONE 셀 컨트롤바 실제 DOM을 인스펙트해 올바른 열삭제 셀렉터 확정. 대안: 2열 데이터를 네이티브 표 대신 '항목 → 내용' 스타일 텍스트로 렌더(모바일 친화·브리틀 회피, 단 quality.py 표 10점 손실 고려).

2. **품질 게이트(quality.py) ↔ §6 목표 글자수 충돌 (2026-06-30 확인)**
   - §6은 카테고리별 목표를 하향(건강 1,200~1,800 / 살림 1,500~2,000)했으나, `generator/quality.py`의 body_length 채점은 여전히 **2,000자+=20점 / 1,500+=10점 / 미만 "너무 짧음" critical**.
   - 그대로면 모바일 최적화로 짧게 쓴 글이 게이트에서 감점·재생성 → §6 의도와 정반대. **quality.py 임계값을 카테고리별 목표에 맞게 동기화 수정 필요.** §9 점수표의 "본문 2,000자 이상" 문구도 함께 갱신.

3. **카테고리별 §6 이식 진행 현황**
   - 정부지원: §6 반영 완료(2,000~2,500 목표, 2열표, 구분선, 요약블록, 마커 체크리스트). 단 위 #1 때문에 표는 아직 3열.
   - 건강: 구분선만 적용. **요약블록·2열 정리표·마커 체크리스트 미적용** → 다음 이식 대상.
   - 살림/일상(`_SYSTEM`), 레시피(`recipe.py`): §6 미반영. 순차 적용 예정.
