...
* 구현: `poster/naver_blog.py`의 `_apply_inline_emphasis`(안전 상한 20회). 반드시 `_convert_bullets_to_list`보다 먼저 실행해 문단 구조가 바뀌기 전에 처리한다.
...
* `poster/infographic_html.py`(`create_infographic_via_html`, info_post.py 전용 — gov/health/stock은 여전히 PIL 기반 `create_health_header_card` 사용) 900×900 정방형, 3단제목+아이콘카드3~4개+CTA 구조.
