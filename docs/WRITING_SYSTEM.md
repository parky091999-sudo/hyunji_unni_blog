...
### ✅ 해결됨 (Resolved)
- **두번째스물하나(ggonnip100) 벤치마킹 — 개념 카드 인포그래픽 ✅렌더러 완성·시각검증 / 배선 DRAFT 대기 (2026-07-06 밤)**: 사용자 제공 벤치마크 = 비즈니스·경제 전문 대형 블로거(이웃 4,925·일방문 500+), 주제가 현지언니와 **거의 100% 겹치는 직접 경쟁자**(퇴직연금·ETF·공모주·정부지원·종목분석·부동산). 6글+목록 분석 결과 **우위는 텍스트가 아니라 커스텀 비주얼 투자**.
  ...
  - **🔜 배선 미완(자동발행 안전상 DRAFT 검증 필수)**: info_post/gov_post에 [사진2] 마커 주입 + 헤더 [사진1] 최상단 보장(모델이 [사진1] 누락 시 `img_idx=마커번호-1`이라 헤더 고아화 방지, [naver_blog.py](poster/naver_blog.py) L914). 앵커=2번째 [구분선] 앞 폴백 [표삽입] 뒤. 푸시→DRAFT로 실삽입 위치·헤더정위치 확인 후 라이브.
  - **🔜 잔여 벤치마크 백로그(우선순위순)**: ⑤신청방법 주석 스크린샷(정부지원·info '어떻게 신청' 신뢰도 최고, Playwright 사이트캡처+빨간원 오버레이 — 사이트별 레이아웃 취약 리스크 중간) / ②컬러 소제목(SE ONE 색상 자동화 불가 §7 → HTML 배너 이미지 우회 검토) / ①에디토리얼 일러스트(최대 엔게이지먼트지만 자동발행 일관성·이미지모델 비용/품질 리스크로 전략검토) / ④마스코트 본문 삽입 / ⑥CTA 배너. 원본 스크린샷: `벤치마크/두번째스물하나/`(+`/부동산`).
