...
3. **카테고리별 §6 이식 진행 현황**
   - 정부지원: §6 반영 완료(2,000~2,500 목표, 1개 3열표, 구분선, 요약블록, 마커 체크리스트, 소제목 번호제거). 실발행 검증 완료(224331602830).
   - 건강: 구분선·소제목 번호제거 적용. **요약블록·3열 정리표·마커 체크리스트 미적용** → 다음 이식 대상.
   - 살림/일상(`_SYSTEM`), 레시피(`recipe.py`): §6 부분 반영. 순차 적용 예정.

4. **글머리기호 내어쓰기(hanging indent) — 미구현 (2026-06-30 지시)**
   - `·`/`①`/`✔` 글머리 줄이 길어 줄바꿈되면 둘째 줄이 좌측 끝으로 돌아가 글머리표 아래로 안 맞음 → 첫 글자 위치 기준 내어쓰기 필요(§5).
   - **구현 후보**: ⒜ 네이버 SE 네이티브 글머리표(•) 리스트로 변환(자동 hanging indent 내장) — 단 SE ONE 리스트 버튼/적용 자동화 probe 필요. ⒝ 텍스트 단락에 내어쓰기 서식 적용 가능 여부 확인. 표 열삭제처럼 SE 합성이벤트 이슈 가능성 → **로컬 headed probe로 먼저 검증**.
