# 작업 인수인계 — 2026-07-24 세션 마감 → 다음 세션

---

## ★★★★★ 2026-07-24 세션 — QC게이트 전면보완 + ops마이그레이션 + 상투어구멍 + 블로그통계분석·개선

> **이 섹션부터.** 시작 루틴: 전 6레포 pull → 각 레포 `data/qc_log.jsonl`(FAIL은 `qc_fail.jsonl`) 확인, `python scripts/qc_digest.py`.

### 1) 07-24 발행분 점검 → 상투어 금지 '구멍 3종' 수정 (`e05f5b5`)
- QC 게이트가 WARN 2건(실업급여 WP 상투어6·AI CLI 네이버 상투어12) 물어옴 → 추적해 근본원인 수정:
  - **해요체 사각지대**: 상투어 금지가 전 생성기에 '-합니다체'로만 있어 해요체('것이 중요해요')가 통과. deep/info/content에 해요체 변형 명시.
  - **tech_guide 트랙 누락**: 뉴스 트랙(tech_content)엔 있던 금지어(혁신적/탁월/극대화/것이중요)가 가이드 트랙에만 빠져 있었음 → 동일화.
  - **🐛 `_AI_SANITIZE` 치환 버그**: '확인하는 것이 중요합니다'→'확인보면 편해요'로 어간 깨짐(정규식이 '하'까지 소비, 치환문에 '해' 없음) → '해보면 편해요' 복구.

### 2) ★자동 QC 게이트 전면 보완 (사용자 '다 반영', 6레포)
- **네이버 라이브 프로브화**: 소스텍스트 대신 PostView 실측 → 이미지/표 렌더유실 탐지(`qc_naver_live`). WP는 기존 self-heal 유지.
- FAIL→`qc_fail.jsonl` 분리 + qc_log **롤링**(800줄, 무한증가 방지). 이미지0·소제목계층역전·본문과소 경고 추가. 상투어 정규식 해요체 포함.
- `scripts/qc_digest.py`(요약)·`scripts/_test_qc.py`(16 assert). **대시보드 타일**(collect.qc_metrics=각 레포 qc_fail.jsonl 24h 집계). **LLM 훅↔상품 심판**(옵트인 `QC_LLM_JUDGE=1`, 기본 off).
- 커밋: hyunji `6642bae`, coupang `024711d`, jiniee `2c5b365`, soyu `d3165a0`, dashboard `477bf0f`. jiniee/soyu는 인라인→모듈화.

### 3) ★EC2 run_*.sh 5종 레포 편입(버전관리) — 부트스트랩 패턴
- EC2 로컬에만 있던 러너 5개를 각 레포 `ops/`로 편입. EC2엔 얇은 부트스트랩(`cd repo && git pull && exec ops/<script> "$@"`)만. 원본 백업=`run_*.sh.pre-ops.20260724`. 파일럿(run_wp_tech) + 확대 전부 체인검증(발행없이 exit=0/venv도달).
...
- **주말 무변경 판단**: 정보 크론 이미 7일 가동 → 07-18/19 딥은 발행공백 아닌 주말 검색수요 감소. 손대는 게 오히려 리스크.

### 🔜 다음 세션
1. **QC 효과 확인**: `qc_digest.py`로 상투어(해요체 fix 후)·FAIL 추이. 특히 다음 info/gov/tech_guide 발행분 상투어 수치.
2. **블로그 통계 재점검**(며칠 뒤): ①방문당 조회수(내부링크 효과) ②청약 색인/검색유입(백로그 소진 효과).
3. ops 부트스트랩 정상동작 확인(오늘 14:05 tech WP·내일 09:05 WP허브가 새 경로로 발행됐는지 로그).
