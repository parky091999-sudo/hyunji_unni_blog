...
## ★★★★★ 2026-07-21 회사 PC 세션 — 당일 발행분 전수 점검 (오류 조치)

> **이 섹션부터 읽으세요.** 오늘은 07-20 대개편(버그5종·발행량확대) 이후 **둘째 실전일**. 전 채널 당일 발행분을 전수 점검했고, 07-20에 고친 것들이 실제로 라이브에 반영됐는지 확인.
> **⚠️ 2차 정밀점검(사용자 5대 기준)에서 형수테크 WP 마크다운 미변환 버그를 잡아 수정·라이브 보정 완료** — 아래 "🔴 2차 정밀점검" 섹션 참조. (1차 텍스트 점검에선 "하드에러 0"이었으나, WebFetch의 마크다운 변환이 리터럴 `**`를 볼드로 렌더해 버그를 가렸음 → raw HTML 실측에서 발견.)
...
### 🔴 2차 정밀점검 (사용자 5대 기준: 이미지 적합성·이미지/표 위치·기호누출·글머리레벨·소제목구성) — 형수테크 WP 마크다운 버그 수정·라이브 보정 완료
...
## ★★★★★ 2026-07-20 회사 PC 세션 — EC2 크론 첫 실전 검증 + 버그 5종 수정 + 발행량 확대
...
- **🐛 청약 인포카드 배치 수정**(이천 휴먼빌 라이브 피드백): "현금 카드"가 다음 소제목(타입) 위=섹션 끝에 붙어 오독. `_own_sub`+`_first_prose_after`로 **자기 소제목 바로 아래(리드인)** 배치. 소제목 겹침에 의한 카드 2장 유실도 해소. 시뮬 3케이스 PASS. §7 0-n, 커밋 273d3ba.
- **🐛 유진 답글봇 Groq 401**: 무효 키 → coupang 정상 키로 EC2 내부 교체(HTTP 200 확인). Gemini가 커버 중이라 서비스 영향은 없었음.
- **유진 첫 EC2 자동발행 성공**: 매직행주 검수·승인(approvals true, af2f2b9) → **12:30 릴스+스레드 발행 완료**(워밍업 모드). 어제 만든 검수 체제 첫 사이클 완주. **→ 유진 재고 다시 0**(오늘 17:30 stock_builder가 새로 렌더·적재 예정).
...
### 추가 (심야 3차 — 07-20 00시 마감, 집 PC 종료 전 정리)
- **tech WP 카테고리 9종 체제**: 네이버 5종 카테고리 생성+헤더 메뉴 재정렬, WP 심층가이드 주제 풀
...
2. **아침 자동발행 첫날 확인**: WP 09:05(가이드 — 새 5종 카테고리 로테이션 시작) · soyu 11:00 ·
   tech_guide 3트랙 · 속보 스캔. `gh run list`로.
3. **형수테크 라이브 사진 정리(수동)**: 폴드8 글 테슬라 모형차 3장 + 갤럭시AI 글 쇼핑 2장 삭제.
4. GSC tech 사이트맵 '성공' 전환 확인(안 되면 재제출). 어제 제출됨, URL 서빙은 정상 확인됨.
5. 새 청약 공고 발행 시 **카드 6종 첫 라이브 검수**(배치·간격·모바일 가독성).
6. 소유(블로그스팟) 사이드바: Contempo→Simple 테마 수동 변경 후 가젯 구성(변경하면 Claude가 이어서).
7. 유진 인스타 프로필 수동 세팅(어제 안내 문구: 이름 '유진｜자취 기록', 소개+Threads 링크).
8. (백로그) 재소싱 필요 2건: 로봇강아지·접이식 소파베드(클립 소실) — AiPrice 반자동.
